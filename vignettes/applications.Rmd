...
ecg.graph <- gfpop::graph(
  gfpop::edge("beforeQ",   "Q", "down", 80000000, gap=0),
  gfpop::edge("Q",         "R", "up",   0,        gap=2000),
  gfpop::edge("R",         "S", "down", 0,        gap=5000),
  gfpop::edge("S",         "S1", "up",   0,        gap=2000),
  gfpop::edge("S1",        "S2", "up",   0,        gap=1000),
  gfpop::edge("S2",        "peak", "up",   0,        gap=0),
  gfpop::edge("peak",      "afterPeak", "down", 0,        gap=0),
  gfpop::edge("afterPeak", "last", "down", 0,        gap=0),
  gfpop::edge("last",      "beforeQ", "up",   0,        gap=0),
  all.null=TRUE)

fit <- gfpop::gfpop(vectData = ECG$data$millivolts, mygraph = ecg.graph, type = "gauss")

library(data.table)
if(interactive()){#tikz code for ecg graph:
  cat(tikz.tex <- data.table(ecg.graph)[type != "null", paste(c("
...
fit <- gfpop::gfpop(
  vectData = neuroSpike$calcium, mygraph = multimodal.graph, type = "gauss")
