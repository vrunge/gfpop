...
dataGenerator <- function(n, changepoints, parameters, type = "mean", sigma = 1, gamma = 1)
{
  ### STOP ###
  if(is.numeric(n) == FALSE || n%%1 != 0){stop('n is not an integer.')}
  if(n < 2){stop('n must be a nonnegative integer')}
  ...
  if(type != "mean" && type != "variance" && type != "poisson" && type != "exp")
  {stop('Argument "type" not appropriate. Choose among "mean", "variance", "poisson", "exp"')}
  ...
  if(type == "variance")
  {
    vectData <- rnorm(n, 0, rep(parameters, SegLength))
  }
